...
class overtime_detail(models.Model):
    ...
    employee = fields.Many2one(comodel_name='hr.employee',string="Tên nhân viên")
    department = fields.Many2one(comodel_name='hr.department',string='Phòng ban')
    date = fields.Date(string='Ngày làm thêm',default=datetime.today())
    ot_rate = fields.Float(string='Tỷ lệ làm thêm')
    ot_time = fields.Integer(string="Số giờ làm thêm", min='0.5',max='16')
